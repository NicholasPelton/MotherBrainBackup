...
def handle_climate_u(msg,num):
    global outlet
    outlet = oselect(num)
    bozo = outlet
    bozo.c_style = msg
    if msg == "/ac":b_text = "A/C"
    if msg == "/heater":b_text = "Heater"
    if msg == "/humidifier":b_text = "Humidifier"
    if msg == "/deHumidifier":b_text = "De-Humidifier"
    print(msg)
    jobomatic(bozo)
    emit(str(num) + 'climate_template', msg, broadcast=True, include_self=False)
    emit(str(num) + 'climate_name', b_text, broadcast=True, include_self=False)

@socketio.on('climate_2')
def handle_climate_2(msg,style,num):
    global outlet
    bozo = oselect(num)
    print (style)
    if style == "day_high":Outlets.Outlet.d_t_high = int(msg)
    if style == "day_low":Outlets.Outlet.d_t_low = int(msg)
    if style == "night_high":Outlets.Outlet.n_t_high = int(msg)
    if style == "night_low":Outlets.Outlet.n_t_low = int(msg)
    if style == "hum_high":Outlets.Outlet.h_high = int(msg)
    if style == "hum_low":Outlets.Outlet.h_low = int(msg)
    if style == "day_t":
        bozo.day_t = msg
        style = str(bozo.num)+style
    if style == "night_t":
        bozo.night_t = msg
        style = str(bozo.num)+style
    emit(str(style), msg, broadcast=True, include_self=False)

@socketio.on('feeding_u')
def handle_feeding_u(msg,style,num):
    bozo = oselect(num)
...
def handle_t_on(msg,num):
    ...
    bozo.timeon = msg
    jobomatic(bozo)
    emit(str(outlet.num) + 't_on', msg, broadcast=True, include_self=False)
...
def handle_t_off(msg,num):
    ...
    bozo.timeoff = msg
    jobomatic(bozo)
    emit(str(outlet.num) + 't_off', msg, broadcast=True, include_self=False)
